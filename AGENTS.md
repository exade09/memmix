...
- On a **sponsored** launch the creator fee goes to the **vault**, and the request cannot redirect it — Fons pays the launch fee and the gas, and that fee is what pays for it. It is the vault's only income: $FONS is launched outside this site, so its own fee never arrives here. On a **self-paid** launch the fee stays with the person who signed, which is what makes "pay it yourself" a real choice rather than a worse deal than going to Pons directly. Both are written into the token at launch and can never be changed, and the launch screen says which applies before anything is sent.
- Two addresses that look interchangeable are not: `token.deployer()` is `msg.sender`, but the **curve** records the creator fee recipient as its creator, and that is what Pons shows and what fees are collected against. Read off a real mainnet launch, not assumed.
- `REWARDS_VAULT_ADDRESS` must be the sponsor wallet. Payouts are signed with the sponsor key and [`execute_distribution`](axiom_scanner/rewards/vault.py) refuses with `WALLET_MISMATCH` otherwise, so a different address collects fees the site can never send.
