...
- On a sponsored launch the creator fee goes to **the launcher's connected wallet**, never to the vault. Fons pays the fee and gas and keeps nothing. This is also the only thing that makes them the creator: the factory takes the deployer from `msg.sender`, which is the sponsor wallet, but the curve records the creator fee recipient as its creator — that is what Pons shows and what fees are collected against. Do not "fix" this by pointing it at the vault; the vault is funded by $FONS's own fee, because $FONS is launched with the vault wallet connected. The *rate* stays ours and uniform, because it taxes everyone who later trades the token and can never be changed after launch.
